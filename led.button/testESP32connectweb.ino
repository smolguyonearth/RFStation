...
void httpTask(void *param) {
  ...
  while (true) {
    ...
    if (xQueueReceive(buttonQueue, &btn, 0) == pdTRUE) {
      // In AP mode, we just try to send.
      // (Assuming the backend PC is connected to this ESP32's WiFi)
      WiFiClient client;
      HTTPClient http;
...
void setup() {
  ...
  xTaskCreatePinnedToCore(httpTask, // Task function
                          "HTTP",   // Name
